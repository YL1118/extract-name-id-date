...
# -*- coding: utf-8 -*-
"""
TXT extractor (Taiwanese admin-style docs)
ID-ONLY name extraction + Unified scoring + Batch + Minimal output

特性：
- 姓名僅透過「身分證錨點」補抓（不走姓名標籤路徑）
- 仍抽 id_no / ref_date / batch_id 於各自標籤附近
- 精簡輸出：value / confidence / context10
- 批次：可指定資料夾
"""
...
LABELS: Dict[str, List[str]] = {
    # 注意：這版不使用 name 標籤抓姓名（只用 ID 錨點），但保留其他欄位照常
...
ENABLE_IDLABEL_PROXIMITY = True         # 姓名距 ID 標籤越近越加分（寫入 context_bonus 再進 base 分）
...
NAME_BLACKLIST_SUBSTRING = True          # True: 子字串命中也排除

# ----- ID-only name extraction 參數 -----
ANCHOR_PROX_WEIGHT = 1.0                 # final_score 加上與 ID 錨點鄰近度的權重
MIN_ID_NAME_PROX = 0.0                   # 最小鄰近門檻（0~1），過濾離 ID 太遠的姓名，0 表示不過濾
...
# ==============================
# Candidate discovery (id_no / ref_date / batch_id)
# ==============================
def find_field_candidates_around_label(field: str, label: LabelHit, lines: List[str]) -> List[Candidate]:
    """這版只處理 id_no/ref_date/batch_id；不在此處產生 name 候選"""
    ...
    def add_candidate(value: str, vcol: int, line: int, dir_key: str, fmt_conf: float) -> None:
        ...
        if dist < (0.2 if field != "ref_date" else 0.1):
            return
    ...
    if field == "id_no":
        ...
        for pat in DATE_PATTERNS:
            for m in pat.finditer(left_seg):
                iso = parse_iso_date(m.group(0))
                if iso:
                    add_candidate(iso, left_start + m.start(), label.line, "same_left", 1.0)
    elif field == "batch_id":
...
def final_score(c: Candidate, anchor: Optional[Candidate] = None) -> float:
    ...
    if anchor is None:
        return base
    prox = anchor_proximity_score(anchor.col, c.col, c.line - anchor.line)
...
# ==============================
# Grouping（以 ID 為唯一 anchor，再抓姓名）
# ==============================
def group_records_id_only_name(all_cands: Dict[str, List[Candidate]],
                               lines: List[str],
                               surname_singles: Set[str],
                               surname_doubles: Set[str],
                               id_label_positions: List[Tuple[int,int]]) \
                               -> List[Tuple[Optional[Candidate], Optional[Candidate], Optional[Candidate], Optional[Candidate]]]:
    """
    回傳 (name_c, id_c, date_c, batch_c)
    姓名只從「ID 錨點」在同列/下方 MAX_DOWN_LINES 行掃描產生候選，再用 unified 分數挑最佳
    """
    out: List[Tuple[Optional[Candidate], Optional[Candidate], Optional[Candidate], Optional[Candidate]]] = []
    id_anchors = sorted(all_cands.get("id_no", []), key=lambda c: (c.line, c.col))

    def pick_best(cands: List[Candidate], anchor: Optional[Candidate]) -> Optional[Candidate]:
        if not cands: return None
        cands_sorted = sorted(cands, key=lambda x: (-final_score(x, anchor), tie_break_key(x, anchor)))
        return cands_sorted[0]

    for idc in id_anchors:
        # 先挑其他欄位
        date_c = pick_best(
            [c for c in all_cands.get("ref_date", []) if abs(c.line - idc.line) <= MAX_DOWN_LINES],
            idc
        )
        batch_c = pick_best(
            [c for c in all_cands.get("batch_id", []) if abs(c.line - idc.line) <= MAX_DOWN_LINES],
            idc
        )

        # 姓名候選（只從 ID 錨點掃描產生）
        name_pool: List[Candidate] = []
        for dl in range(0, MAX_DOWN_LINES + 1):
            ...
            for name, col in name_candidates_from_text(lines[li], surname_singles, surname_doubles):
                if is_name_blacklisted(name):
                    continue
                prox = distance_score(idc.col, col, li - idc.line)
                if prox < MIN_ID_NAME_PROX:
                    continue
                name_cand = Candidate(
                    ...
                    format_conf=0.7, label_conf=0.4, dir_prior=0.6, dist_score=prox,
                    context_bonus=0.0
                )
                # 額外：姓名距「ID 標籤」加分（進 base 分）
                if ENABLE_IDLABEL_PROXIMITY and id_label_positions:
                    best = 0.0
                    for li2, lc2 in id_label_positions:
                        best = max(best, distance_score(lc2, name_cand.col, name_cand.line - li2))
                    name_cand.context_bonus += IDLABEL_BONUS_SCALE * best
                name_pool.append(name_cand)

        name_c = pick_best(name_pool, idc)
        out.append((name_c, idc, date_c, batch_c))
...
def extract_minimal_from_text(text: str, surname_txt_path: Optional[str] = None) -> Dict:
    ...
    # 2) 依標籤產生候選（僅 id_no/ref_date/batch_id）
    all_cands: Dict[str, List[Candidate]] = {"name": [], "id_no": [], "ref_date": [], "batch_id": []}
    for h in label_hits:
        cands = find_field_candidates_around_label(h.field, h, lines)
        all_cands[h.field].extend(cands)

    # 用於「姓名距 ID 標籤加分」
    id_label_positions: List[Tuple[int,int]] = [(h.line, h.col) for h in label_hits if h.field == "id_no"]

    # 3) 以 ID 為唯一 anchor，掃描產生姓名候選並挑選
    grouped = group_records_id_only_name(all_cands, lines, surname_singles, surname_doubles, id_label_positions)
    ...
    minimal_records = []
    for name_c, id_c, date_c, batch_c in grouped:
    ...
    # 簡短報告（可刪）
    report: Dict[str, List[str]] = {"id_no": [], "ref_date": [], "batch_id": [], "name": []}
    for field in ["id_no", "ref_date", "batch_id"]:
        if not all_cands[field]:
            report[field].append("有標籤但無候選或標籤未偵測")
        else:
            report[field].append(f"候選 {len(all_cands[field])} 條")
    # name 只靠 ID，若沒有 ID 就不會有 name
    if not all_cands["id_no"]:
        report["name"].append("因未偵測到身分證，姓名不執行補抓")
...
def main(argv: List[str]) -> None:
    import argparse
    ap = argparse.ArgumentParser(description="TXT extractor (ID-only name, unified scoring, minimal output) with batch support")
