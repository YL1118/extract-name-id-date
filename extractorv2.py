...
# NEW: Extra proximity-to-ID boost for names (with ambiguity suppression)
W_NEAR_ID = 0.8          # strength of "near to an ID" on name score
ID_NEAR_MAX_LINES = 2    # only consider IDs within +/- this many lines
ID_NEAR_TAU_COL = 10.0   # column distance scale for name–ID proximity
ID_SEPARATION_GAMMA = 0.25  # margin scale between top-1 and top-2 proximity; smaller → stronger penalty
...
class Candidate:
    ...
    penalty: float = 0.0
    near_id: float = 0.0  # NEW: proximity to nearest ID after ambiguity suppression (0..1)
...
def find_label_hits(lines: List[str], labels: Dict[str, List[str]], max_edit: int = 1) -> List[LabelHit]:
    ...
    for li, line in enumerate(lines):
        ...
        # fuzzy matches on short alnum/CJK spans
...
def name_candidates_from_text(line_text: str, surname_singles: Set[str], surname_doubles: Set[str]) -> List[Tuple[str, int]]:
    """Return list of (name, col) candidates:
    - Prefer double surnames, else single
    - Given name = next two CJK chars (skip separators)
    - No total-length constraint; robust to glued tokens
    """
...
# ==============================
# Name–ID proximity bonus with ambiguity suppression
# ==============================
def _ambiguity_suppression_factor(best: float, second: float, gamma: float = ID_SEPARATION_GAMMA) -> float:
    """
    Return a factor in [0,1]. If (best - second) is small (ambiguous), factor → 0.
    If separation is large, factor → 1.
    Piecewise-linear clamp: max(0, min(1, (best - second)/gamma))
    """
    margin = max(0.0, best - second)
    return max(0.0, min(1.0, margin / max(1e-6, gamma)))

def apply_name_near_id_bonus(all_cands: Dict[str, List[Candidate]]) -> None:
    """For each name candidate, compute proximity to the nearest ID candidate (0..1)
    with ambiguity suppression if multiple IDs are similarly close.
    - Only consider IDs within +/- ID_NEAR_MAX_LINES lines.
    - Base proximity uses distance_score(..., tau=ID_NEAR_TAU_COL).
    - Weight each ID by its format_conf (valid checksum IDs weigh more).
    """
    id_list = all_cands.get("id_no", [])
    if not id_list:
        return

    for nc in all_cands.get("name", []):
        scores: List[float] = []
        for ic in id_list:
            line_delta = ic.line - nc.line
            if abs(line_delta) > ID_NEAR_MAX_LINES:
                continue
            prox = distance_score(nc.col, ic.col, line_delta, tau=ID_NEAR_TAU_COL)
            # Emphasize valid IDs: 0.5 ~ 1.0 scaling (0.5 for weak, 1.0 for strong)
            weight = 0.5 + 0.5 * max(0.0, min(1.0, ic.format_conf))
            scores.append(prox * weight)
        ...
        best = scores[0]
        second = scores[1] if len(scores) >= 2 else 0.0
        factor = _ambiguity_suppression_factor(best, second, ID_SEPARATION_GAMMA)
        nc.near_id = best * factor  # shrink if ambiguous (top-1 ~ top-2)
...
def assemble_record(name_c: Optional[Candidate], id_c: Optional[Candidate], date_c: Optional[Candidate], batch_c: Optional[Candidate], all_cands: Dict[str, List[Candidate]]) -> Record:
    def field_result_from_cand(c: Optional[Candidate], fallback_notes: List[str]) -> FieldResult:
        ...
        return FieldResult(
            value=c.value,
            confidence=max(0.0, min(1.0, c.score()/3.0)),  # rough normalization
            source={
                "line": c.line,
                "col": c.col,
                ...
                "score_breakdown": {
                    ...
                    "penalty": c.penalty,
                    "near_id": getattr(c, "near_id", 0.0),  # NEW: expose for debugging
                }
            },
            notes=[],
        )
...
def extract_from_text(text: str, surname_txt_path: Optional[str] = None) -> Dict:
    ...
    # 3) Apply name–ID proximity bonus with ambiguity suppression
    apply_name_near_id_bonus(all_cands)
    ...
    output = {
        ...
        "meta": {
            ...
            "config": {
                ...
                "near_id": {
                    "ID_NEAR_MAX_LINES": ID_NEAR_MAX_LINES,
                    "ID_NEAR_TAU_COL": ID_NEAR_TAU_COL,
                    "ID_SEPARATION_GAMMA": ID_SEPARATION_GAMMA,
                }
            }
        }
    }
